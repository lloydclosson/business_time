...
class TestTimeExtensions < Test::Unit::TestCase
  ...
  should 'know a weekend day is not a workday' do
    assert( Time.workday?(Time.parse('April 9, 2010 10:45 am'), @config))
    assert(!Time.workday?(Time.parse('April 10, 2010 10:45 am'), @config))
    assert(!Time.workday?(Time.parse('April 11, 2010 10:45 am'), @config))
    assert( Time.workday?(Time.parse('April 12, 2010 10:45 am'), @config))
  end

  should 'know a weekend day is not a workday (with a configured work week)' do
    @config.work_week = %w[sun mon tue wed thu]
    assert( Time.weekday?(Time.parse('April 8, 2010 10:30am'), @config))
    assert(!Time.weekday?(Time.parse('April 9, 2010 10:30am'), @config))
    assert(!Time.weekday?(Time.parse('April 10, 2010 10:30am'), @config))
    assert( Time.weekday?(Time.parse('April 11, 2010 10:30am'), @config))
  end
  ...
  should 'know a holiday is not a workday' do
    ...
    assert(!Time.workday?(Time.parse('July 4th, 2010 1:15 pm'), @config))
    assert(!Time.workday?(Time.parse('July 5th, 2010 2:37 pm'), @config))
  end
  ...
  should 'calculate business time between different times on the same date (clockwise)' do
    ...
    assert_equal time_a.business_time_until(time_b, @config), 260.minutes
  end

  should 'calculate business time between different times on the same date (counter clockwise)' do
    time_a = Time.parse('2012-02-01 10:00')
    time_b = Time.parse('2012-02-01 14:20')
    assert_equal time_b.business_time_until(time_a, @config), -260.minutes
  end

  should 'calculate business time only within business hours even if second endpoint is out of business time' do
    time_a = Time.parse('2012-02-01 10:00')
    time_b = Time.parse('2012-02-01 ' + @config.end_of_workday) + 24.minutes
    first_result = time_a.business_time_until(time_b, @config)
    time_b = Time.parse('2012-02-01 '+ @config.end_of_workday)
    second_result = time_a.business_time_until(time_b, @config)
  ...
  should 'return correct time between two consecutive days' do
    ...
    working_hours = Time.parse(@config.end_of_workday) - Time.parse(@config.beginning_of_workday)
    assert_equal time_a.business_time_until(time_b, @config), working_hours
  end

  should 'calculate proper timing if there are several days between' do
    time_a = Time.parse('2012-02-01 10:00')
    time_b = Time.parse('2012-02-09 11:00')
    duration_of_working_day = Time.parse(@config.end_of_workday) - Time.parse(@config.beginning_of_workday)
    assert_equal time_a.business_time_until(time_b, @config), 6 * duration_of_working_day + 1.hour
    assert_equal time_b.business_time_until(time_a, @config), -(6 * duration_of_working_day + 1.hour)
  end
  ...
  should 'calculate proper duration even if the end date is on a weekend' do
    ...
    assert_equal ticket_reported.business_time_until(ticket_resolved, @config), 6.hours + 20.minutes
  end
  ...
  should 'roll to the end of the same day when after hours on a workday' do
    ...
    workday_end = @config.end_of_workday
    expected_time = Time.parse('#{workday_end} UTC, Wednesday 9th May, 2012')
    assert_equal Time.roll_backward(time, @config), expected_time
  end
  ...
  should 'roll to the end of the previous day when before hours on a workday' do
    ...
    workday_end = @config.end_of_workday
    expected_time = Time.parse('#{workday_end} UTC, Tuesday 8th May, 2012').utc
    assert_equal Time.roll_backward(time, @config), expected_time
  end
  ...
  should 'returns the given time during working hours' do
    ...
    assert_equal Time.roll_backward(time, @config), time
  end
